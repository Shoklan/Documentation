...
class NewMenuDemo(Frame):                                             # an extended frame
   def __init__(self, parent=None):                                   # attach to toplevel?
      ...
      # self.master.inconname("TkPython")                             # label when iconified /Don't have

   def createWidgets(self):
      self.makeMenuBar()
      self.makeToolBar()
      label = Label(self, text='Menu and Toolbars Demo')
      label.config(releif=SUNKEN, width=40, height=10, bg='white')
      label.pack(expand=YES, fill=BOTH)

   def makeToolBar(self):
      toolbar = Frame(self, cursor='hand2', relief=SUNKEN, bg='white')
   ...
   def editMenu(self):
      ...
      pulldown.add_command(label='Spam',    command = self.greetings)
      pulldown.add_separator()
      pulldown.add_command(label='Delete',  command = self.greetings)
   ...
   def notdone(self):
      showerror('Not Implemented', 'This feature is not ready yet')
   def quit(self):
      if askyesno('Verify Quit', 'Are you sure you wish to quit?: '):
         Frame.quit(self)
