##############################################################
# add common edit tool to scrolled text by inheritance;
# composition (embedding) would workd just as well here;
...
from quitter        import Quitter                           # here, not Python's
      ...
      if target:
         ...
         if where:
          ...
          # self.text.tag_remove(SEL, '1.0', END)            # remove selection
            self.text.tag_add(SELF, where, pastit)           # select found target
            self.text.mark_set(INSERT, pastit)               # set insert mark
            self.text.see(INSERT)                            # scoll display
