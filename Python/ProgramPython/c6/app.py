##########################################################################
# an application class hierarchy, for handling top-level componenets;
...
class App:                                                # the root class
   ...
   def help(self):
      print self.name, 'command-line arguements:'         # extended in subclass
      print '-v (verbose)'
   ...
   def getarg(self, tag, default=None):
      try:
         ...
         return default                                   # Non: missing, no default
   ...
   def endargs(self):
      if self.args:
         self.message('extra arguements ignored: ' + repr(self.args))
         self.args = []
   def restargs(self):
   ...
   def main(self):
      ...
      try:
         self.start()
         self.run()
         res = self.stop()                          # optional return value
