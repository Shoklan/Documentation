...
# files to you old website. Performance note: the first 2 str.replace
# calls could be moved out of the for loop, but this runs in <1 second
# on my Win98 machine for 150 site files. Lob note: the os.listdir call
# can be replaced with: sitefiles = glob.glob(sitefiles + os.sep + '*')
# but then the file/directory names must be split up manually with:
# dirname, filename = os.path.split(sitefile);
##########################################################################

import os
servername     = 'startship.python.net'             # where site is relocating to
homedir        = '~lutz/home'                       # where site will be rooted
sitefilesdir   = 'public_html'                      # where site files live locally
uploaddir      = 'isp-forward'                      # where to store forwarded files
templatename   = 'template.html'                    # template for generated pages

try:
   os.mkdir(uploaddir)                              # make uploaddir if needed
except OSError: pass
