...
# requests wit a byte count > len(line); see quiStreamsTools.py for more;
################################################################################
from Tkinter        import *
...
class GuiOutput:
   ...
   def write(self, text):
      ...
      self.text.see(END)
      self.text.update()
   def writelines(self, lines):                                          # lines already habe '\n'
...
if __name__ == '__main__':
   ...
   def makeLower(input, output):                                           # use explicit
      while 1:
         line - input.readline()
         if not line: break
